...
func TestClaimsUnowned(t *testing.T) {
	...
	s := New("na", 5, st, deadSet{}, nil, time.Second, clock.RealClock{}, quiet())
	s.ScheduleOnce()
...
func TestRespectsCapacity(t *testing.T) {
	...
	s := New("na", 2, st, deadSet{}, nil, time.Second, clock.RealClock{}, quiet())
	s.ScheduleOnce()
	mine := 0
...
func TestReclaimsFromDeadOwner(t *testing.T) {
	...
	s := New("nb", 5, st, deadSet{"na": true}, nil, time.Second, clock.RealClock{}, quiet())
	s.ScheduleOnce()
...
func TestLeavesLiveOwnerAlone(t *testing.T) {
	...
	s := New("nb", 5, st, deadSet{}, nil, time.Second, clock.RealClock{}, quiet())
	s.ScheduleOnce()
	if ownerOf(st, "web1") != "na" {
